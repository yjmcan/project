...
Page({
  ...
  formSubmit: function (e) {
    ...
    var txmoney = Number(this.data.fxset.tx_money), je = e.detail.value.je, name = e.detail.value.name, zh = e.detail.value.zh, cb = e.detail.value.checkbox.length, zhlx = e.detail.value.radiogroup;
    console.log(sjdsjid,useryj,txmoney,je,name,zh,cb,zhlx)
    if(zhlx==''){
      wx.showModal({
        title: '提示',
        content: '请选择提现方式',
    ...
    if (useryj <txmoney) {
      ...
      app.util.request({
        ...
        'cachetime': '0',
        data: { store_id: sjdsjid, type: actype, name: name, username: zh, tx_cost: je},
