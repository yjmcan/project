  ...
  onLoad: function (options) {
    ...
    app.util.request({
      ...
      success: res => {
        console.log(res)
        var list = res.data
        list.content = app.convertHtmlToText(list.content)
        var imgs = list.thumb;
  ...
  upfile: function (e) {
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        that.uploadimg({
          url: msg.siteroot + '?i=' + msg.uniacid + '&c=entry&a=wxapp&do=upload&m=zh_wxc',
          path: imgsrc
        });
      }
    })
    ...
    wx.uploadFile({
      ...
      success: (resp) => {
        if (resp.data != '') {
          ...
          console.log(that.data.imgArray1)
