...
Page({
  ...
  delete1: function (e) {
    ...
    console.log('删除images1里的图片后剩余的图片', imgArray1)
    that.setData({
      images1: images1
    });
  },
  ...
  formSubmit: function (e) {
    ...
    console.log(imglogo, imgArray, imgArray1, mdtp,lbtp)
    var mdlogo = imglogo;
    ...
    if (mdlogo == "") {
      ...
      app.util.request({
        ...
        'cachetime': '0',
        data: {
          md_id: mdid, link_name: lxr, yz_tel: sjh, address: mdwz, md_name: mdmc, md_logo: mdlogo, start_time: stime, end_time: etime,
          type_id: hyid, WiFi: wifi, park: tcw, apy: zfbzf, wei: wxzf, notice: mdgg, imgs: mdtp, lb_imgs:lbtp, coordinates: jwd, link_tel: mddh
        },
        success: function (res) {
          console.log(res.data)
          if (res.data == 1) {
  ...
  onLoad: function (options) {
    ...
    var uid = wx.getStorageSync('UserData').id, sjid = wx.getStorageSync('store_id');
    var that = this;
    console.log(getApp().imglink, getApp().getuniacid,uid,sjid)
    //取短信设置
    app.util.request({
      ...
      success: function (res) {
        ...
        if (res.data != '') {
          ...
          if (res.data.is_check == '2') {
            imglogo = res.data.md_logo, imgArray = res.data.newimg,lbimgArray=res.data.lb_newimg;
            //取imgurl
            app.util.request({
            ...
            that.setData({
              ...
              isbj: true,
              isyz: false,
              logo: res.data.md_logo,
              lxr: res.data.link_name,
              sjh: res.data.yz_tel,
              weizhi: res.data.address,
              jwd: res.data.coordinates,
              mdmc: res.data.md_name,
              mddh: res.data.link_tel,
              timestart: res.data.start_time,
              timeend: res.data.end_time,
              mdgg: res.data.notice,
              zsnum: parseInt(res.data.notice.length),
              images: res.data.newimg,
              lbimages:res.data.lb_newimg
            })
            console.log('imgArray', imgArray, 'imgArray1', imgArray1, 'images', that.data.images, 'images1', that.data.images1, 'lbimgArray', lbimgArray, 'lbimgArray1', lbimgArray1, 'lbimages', that.data.lbimages, 'lbimages1', that.data.lbimages1)
          }
        }
