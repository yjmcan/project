...
Page({
  ...
  ljsy: function (e) {
    var yhqid = e.currentTarget.dataset.yhqid, qid = e.currentTarget.dataset.qid;
    console.log(e, yhqid, qid)
    wx.navigateTo({
      url: 'hxq?yhqid=' + yhqid + '&qid=' + qid,
    })
    // wx.showModal({
    //   title: '提示',
    //   content: '正在开发中',
  ...
  reLoad: function () {
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        var kyd = [], sxd = [];
        for (var i = 0; i < all.length; i++) {
          if (!(util.validTime1(nowtime, all[i].end_time)) || all[i].state == '2' || all[i].state == '3') {
