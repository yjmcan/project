...
Page({
  ...
  caomfirm: function () {
    var user_id = wx.getStorageSync('users').id, myaddress = this.data.myaddress;
    var that = this;
    ...
    console.log(myaddress,user_id, goodid, Number(integral),hb_moeny,total_score,good_name,good_img)
    if (that.data.spinfo.type == '1'){
      ...
      if (Number(integral) > total_score) {
        ...
        var lxr = myaddress.user_name, tel = myaddress.tel, dz = myaddress.area + myaddress.address
        console.log(lxr,tel,dz)
        wx.showLoading({
          title: '提交中',
          mask: true,
        })
