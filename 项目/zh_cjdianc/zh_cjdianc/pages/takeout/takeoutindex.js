...
Page({
  ...
  scsj: function (e) {
    var that = this, issc = this.data.issc == '2' ? '1' : '2', user_id = wx.getStorageSync('users').id, store_id = that.data.store_id;
    console.log(issc, user_id, store_id)
    //SaveCollection
    app.util.request({
      'url': 'entry/wxapp/SaveCollection',
      'cachetime': '0',
      data: {
        store_id: store_id, user_id: user_id, type: issc
      },
  ...
  submit: function () {
    ...
    if (userinfo.img == '' || userinfo.name == '') {
      ...
      wx.navigateTo({
        url: 'takeoutform?storeid=' + that.data.store_id,
      })
    }
  },
  lookck: function () {
  ...
  cartadd: function (e) {
    ...
    console.log(itemIndex, parentindex, goodid, cart_list, spinfo, user_id, store_id, this.data.iszk, isvipm)
  ...
  ggaddcart: function () {
    var itemIndex = this.data.itemIndex, parentindex = this.data.parentindex, dishes = this.data.dishes, spinfo = this.data.dishes[parentindex].good[itemIndex], cart_list = this.data.cart_list.res, that = this, gginfo = this.data.gginfo, user_id = wx.getStorageSync('users').id, r = this.data.gg, store_id = this.data.store_id;
    var c = [],
      u = !0;
    for (var n in r) {
      var l = !1;
    ...
    console.log('加入购物车', itemIndex, parentindex, dishes, cart_list,spinfo, gginfo, user_id, store_id, r, c, c.toString())
    // var num = 1
    // console.log(that.isInArray(cart_list, gginfo.good_id))
    // if (spinfo.start_num != '0' && !that.isInArray(cart_list, gginfo.good_id)) {
    //   num = Number(spinfo.start_num)
    // }
    // console.log(num)
    u && (wx.showLoading({
  ...
  onLoad: function (options) {
    ...
    //
    this.setData({
      params: { store_id: options.storeid, type: '全部', img: '' }
    })
    this.getstorelist()
    //
